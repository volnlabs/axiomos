...
run_step v04-behavior-runner-smoke python3 -c \
    'import subprocess,sys; runner=[sys.executable,"scripts/benchmark/v04-behavior-runner.py","--behavior","smoke","--sample-id","1"]; stages=["load","verify","admit","attach","active"]; args=sum((["--"+stage,"true"] for stage in stages),[]); ok=subprocess.run(runner+args,capture_output=True,text=True); lines=[line.split(" stage=",1)[1].split()[0] for line in ok.stdout.splitlines() if line.startswith("V04_BEHAVIOR")]; assert ok.returncode==0 and lines==stages, (ok.returncode,lines); bad=subprocess.run(runner+["--load","false"]+sum((["--"+stage,"true"] for stage in stages[1:]),[]),capture_output=True,text=True); assert bad.returncode != 0 and "V04_BEHAVIOR" not in bad.stdout, bad.stdout; bad=subprocess.run(runner+["--load","true","--verify","false"]+sum((["--"+stage,"true"] for stage in stages[2:]),[]),capture_output=True,text=True); assert bad.returncode != 0 and all("stage="+stage not in bad.stdout for stage in stages[2:]), bad.stdout; invalid=runner.copy(); invalid[invalid.index("smoke")]="bad name"; bad=subprocess.run(invalid+args,capture_output=True,text=True); assert bad.returncode != 0 and "V04_BEHAVIOR" not in bad.stdout, bad.stdout'
