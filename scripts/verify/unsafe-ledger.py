...
def render_markdown(sites: list[Site], obligations: tuple[Obligation, ...]) -> str:
    ...
    return f"""<!-- Generated by scripts/unsafe-ledger.py; edit ledger.toml instead. -->
# Unsafe-code ledger
...
def validate(
    ...
    if not LEDGER_PATH.exists() or LEDGER_PATH.read_text(encoding="utf-8") != generated:
        errors.append(f"{LEDGER_PATH.relative_to(ROOT)} is stale; run scripts/unsafe-ledger.py --write")
    return errors
