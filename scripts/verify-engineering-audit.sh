...
run_step bpf-module-boundaries-static python3 -c \
    'from pathlib import Path; root=Path("kernel/src/bpf"); manager=(root / "mod.rs").read_text(); handles=(root / "handles.rs").read_text(); limits=(root / "limits.rs").read_text(); trust=(root / "trust.rs").read_text(); assert all(f"mod {name};" in manager for name in ("handles", "limits", "trust")); assert all(token not in manager for token in ("PRODUCTION_BPF_TRUSTED_KEY", "struct BpfLimits", "fn signing_policy", "fn encode_handle", "BPF_HANDLE_MAX_GENERATION")); assert all(token in handles for token in ("fn encode", "fn decode", "fn insert", "fn can_reuse", "stale_generation_does_not_decode_after_slot_reuse")); assert "struct BpfLimits" in limits and limits.count("for_active_profile") == 2; assert "include_bytes!" in trust and "bpf-unsigned-development" in trust and "fn signing_policy" in trust'
