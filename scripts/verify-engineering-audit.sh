...
run_step bpf-map-resize-fault-static python3 -c \
    'from pathlib import Path; source=Path("kernel/crates/kernel_bpf/src/maps/timeseries.rs").read_text(); assert "fn resize_with_reservation" in source; assert "timeseries_resize_fail_after_n_preserves_storage" in source; assert "for fail_at in 1..=1" in source; test=source.split("fn timeseries_resize_fail_after_n_preserves_storage", 1)[1]; assert all(token in test for token in ("before_buffer", "before_entries", "MapError::OutOfMemory", "storage.head_idx"))'
