...
run_step wait-channel-static python3 -c \
    'from pathlib import Path; chan=Path("kernel/src/mcore/mtask/scheduler/wait_channel.rs").read_text(); wait=Path("kernel/src/mcore/mtask/scheduler/wait.rs").read_text(); mod_text=Path("kernel/src/mcore/mtask/scheduler/mod.rs").read_text(); assert "mod wait_channel" in mod_text, "wait_channel module must be declared in scheduler/mod.rs"; assert chan.count("changed_since(observed_generation)") >= 2, "generic WaitChannel::park must check changed_since(observed_generation)"; assert "self.waiters.enqueue(item)" in chan, "generic WaitChannel::park must call WaiterSink::enqueue"; assert "self.waiters.try_take()" in chan, "generic WaitChannel::drain_waiters must call WaiterSink::try_take"; assert "self.waiters.on_wake(item)" in chan, "generic WaitChannel must call WaiterSink::on_wake"; assert all(token not in chan for token in ("Mutex", "RwLock", "Vec", "log::")), "no kernel-internals leakage in generic wait_channel"; assert all(token not in wait for token in ("Mutex", "RwLock", "Vec", "log::")), "no kernel-internals leakage in production wait"; assert "impl Drop" not in chan, "no custom Drop impl in wait_channel (compiler-generated drop_glue only)"; assert "impl Drop" not in wait, "no custom Drop impl in wait (compiler-generated drop_glue only)"; assert "State::Waiting" in mod_text and "registration.park(zombie_task)" in mod_text, "scheduler integration unchanged"'
