...
run_step bpf-handle-fault-static python3 -c \
    'from pathlib import Path; source=Path("kernel/src/bpf/handles.rs").read_text(); harness=Path("kernel/tests/bpf_handles_fault.rs").read_text(); assert "fn insert_with_reservations" in source; assert source.count("try_reserve(1)") == 2; assert "append_reservation_failure_sweep_preserves_state_and_value_ownership" in source; assert "for fail_at in 1..=2" in source; assert "published a slot" in source and "published a generation" in source; assert "#[path = \"../src/bpf/handles.rs\"]" in harness, "host harness must execute the production handle module"'
run_step bpf-handle-fault-test-build rustc --edition 2021 -D warnings --test \
    kernel/tests/bpf_handles_fault.rs -o "$OUTPUT_DIR/bpf-handle-fault-tests"
run_step bpf-handle-fault-tests "$OUTPUT_DIR/bpf-handle-fault-tests"
