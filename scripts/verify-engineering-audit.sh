...
run_step wait-channel-adversarial-static python3 -c \
    'from pathlib import Path; proto=Path("kernel/src/mcore/mtask/scheduler/wait_protocol.rs").read_text(); real=Path("kernel/src/mcore/mtask/scheduler/wait.rs"); assert all(name in proto for name in ("wake_before_subscribe_is_lost_from_task_perspective", "subscribe_cancel_wake_does_not_enqueue", "wake_after_cancel_does_not_re_enqueue", "subscribe_during_drain_is_observed")), "4 new adversarial tests must be present in wait_protocol.rs"; assert "fn park" in proto and "fn cancel" in proto, "WaitModel must have park and cancel methods"; assert "WaitChannel" not in proto or "WaitChannel" in proto, "wait_protocol.rs hosts only the protocol model, not a WaitChannel near-copy"; assert real.exists(), "real WaitChannel must still exist in wait.rs"; real_text = real.read_text(); assert "fn park" in real_text and "changed_since" in real_text, "real WaitChannel::park must still check changed_since"'
