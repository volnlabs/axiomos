...
run_step child-wait-static python3 -c \
    'from pathlib import Path; syscall=Path("kernel/src/syscall/process.rs").read_text().split("pub fn sys_waitpid", 1)[1]; fork=Path("kernel/src/mcore/mtask/process/fork.rs").read_text(); lifecycle=Path("kernel/src/mcore/mtask/process/lifecycle.rs").read_text(); tree=Path("kernel/src/mcore/mtask/process/tree.rs").read_text(); assert "TaskWait::block_current" in syscall and all(token not in syscall for token in ("enable_and_hlt", ".reschedule()", "TODO: Use a proper wait queue")); assert "let _tree = process_tree().write()" in lifecycle and "parent_exit_wait.wake_all()" in lifecycle; assert fork.index("let child_task = Task::fork") < fork.index("self.publish_child(child.clone())"); assert "child process published twice" in tree'
