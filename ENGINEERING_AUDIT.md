...
- **Local required gate evidence at `05a1746`:**
  - **Default mode** (`scripts/verify-engineering-audit.sh`): **77 PASS / 1 SKIP / 0 FAIL**. The SKIP is `audit-fault-injection-qemu-smoke` (`RUN_AUDIT_FAULT not set`); every other step ran and passed.
  - **`RUN_AUDIT_FAULT=1` mode** (`RUN_AUDIT_FAULT=1 scripts/verify-engineering-audit.sh`): **78 PASS / 0 SKIP / 0 FAIL**. The `audit-fault-injection-qemu-smoke` step runs and passes; the full fault-injection coverage of physical allocation, mapper rollback, and exec/spawn failure paths is exercised.
