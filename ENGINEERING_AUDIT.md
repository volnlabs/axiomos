...
- [~] Publish per-crate line/branch coverage and add parser/verifier mutation
  ...
  required line/branch baselines cover 16 host-testable components, including
  ...
  standalone `rk_bridge`; versioned mutation floors cover the BPF verifier,
  ELF parser/loader, full `shrike_link` protocol/state-machine crate, and
  bounded physical-memory `region.rs` scope. Six `deferred-host` components
  still need measured coverage baselines.
