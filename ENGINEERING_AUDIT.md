...
| 7 | Uniform error policy | boot, syscall, VM, VFS, BPF, platform owners | Architecture + kernel refactor | Every supported boundary conforms to ADR-0002 with typed errors and explicit panic/halt policy | Boot, filesystem initialization, and driver edges now have typed errors plus the required `error-policy-static` check. Legacy VM/address-space `Result<_, &'static str>` and low-level unit-error constructors remain. |
...
- [~] Define consistent kernel, syscall, VM, BPF, and boot error policies.
  ...
  Legacy VM/address-space string errors and low-level unit-error address
  constructors still prevent uniform closure.
