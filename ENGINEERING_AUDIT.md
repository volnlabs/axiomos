...
- **Post-audit update (2026-07-13) — current `audit/dev-remediation` branch state:** the historical finding text below remains the evidence for audited commit `661d5ede6331c5ee62d6642451ce63ce1e0d5adf`; this amendment records subsequent remediation without silently rewriting that snapshot.
  - **Closed findings/safety paths:** C-01 user copies validate every mapped page and permission while the address-space guard is held, invalid user pointers return `EFAULT`, and the release QEMU probe covers write/writev/open/clock/nanosleep; C-03 now uses `RawProgram` → `VerifiedProgram` typestate, lifetime-bound `BpfContext<'a>`, and sealed POD contexts; C-04 uses guarded per-CPU interpreter stacks and serializes escaped map-pointer lifetimes against userspace mutation/unload; C-06 removes the RWX JIT allocator from the kernel and erases the AArch64 JIT from every shipped profile; H-05 has fully fallible parsing/loading, 18 malformed-input integration tests plus 1 unit test, and an isolated fuzz target/CI job.
  - **Material partial remediation:** C-02 removes logging re-entry through an atomic current PID but retains the `UnsafeCell` scheduler API; C-05 adds allocation-free transactional map rollback, protection/W^X propagation, in-place transactional remap, owned-frame rollback, and non-wrapping `u32` frame references, but still lacks active-CPU tracking and TLB shootdown; C-07 adds checked map allocation, hard global object/byte/dimension quotas, pin/unpin, detach/unload/destroy, and synchronous busy checks, but not per-process charging or generation handles; H-01 fixes full x86 context writeback, `ENOSYS`, user-fault task termination, typed exec errors, and force-unlock teardown, but still lacks the architecture-neutral lifecycle/result model and end-to-end fault/exit/wait coverage; H-02 fixes clock units, overflow-safe conversion, clock IDs, and timespec validation but still polls for sleep; H-03 removes per-fire heap allocation and caps fanout but still takes a global runtime/manager lock.
  - **CI and governance:** H-06 workflow fixes cover disk/OVMF discovery, explicit embedded targets, Lean PATH setup, the expanded host crate/profile matrix, ELF fuzzing, and a QEMU gate that requires boot, `EFAULT`, and `ENOSYS` markers; Q-04 now has a generated, exact-fingerprint unsafe ledger with every first-party Rust `unsafe` site assigned to an owner/invariant/test obligation. Local release build, both kernel targets, both complete BPF profiles, strict BPF Clippy, the focused syscall/user-memory/time/ELF suites, and the release QEMU smoke are green. Hosted GitHub Actions have not been rerun.
  - **Still open:** C-02 scheduler ownership, C-05 SMP TLB shootdown, H-01 lifecycle integration, H-02 timer wait queues, H-03 lock-free/RCU dispatch, and H-04 credentials/capabilities plus signed-only key provisioning remain release work. The engineering score remains **3/10** and production readiness remains **NO-GO**.
