...
- **Implementation re-audited at:** `111337f` (uniform typed-error enforcement through process and AArch64 memory boundaries) on top of the completed process-invariant split and the previously recorded exec/spawn rollback, mapping transaction, QEMU, concurrency-model, and required-Miri work
...
- **Local required gate evidence at `111337f`:**
...
- **Required local Miri evidence:** `miri-bpf-cloud` runs by default in the normal/full and extended local gates (`f5338dc`). At `111337f`, `cargo miri test --locked -p kernel_bpf --no-default-features --features cloud-profile` passes **427 tests** (43 ignored) with zero UB in both full modes. `--quick` is the documented iteration mode that omits Miri.
