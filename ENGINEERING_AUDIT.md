...
| 7 | Uniform error policy | boot, syscall, VM, VFS, BPF, platform owners | Architecture + kernel refactor | Every supported boundary conforms to ADR-0002 with typed errors and explicit panic/halt policy | Boot, filesystem initialization, driver edges, low-level address constructors, exec preflight, process-memory cloning, process fork, and address-space fork now have typed errors plus the required `error-policy-static` check. Architecture-specific AArch64 paging/DTB string errors remain. |
...
- [~] Define consistent kernel, syscall, VM, BPF, and boot error policies.
  ...
  kernel callers. Architecture-specific AArch64 paging/DTB string errors still
  prevent uniform closure.
