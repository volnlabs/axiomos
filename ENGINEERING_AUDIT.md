...
- **Local required gate evidence at `c24f213`:**
  - **Default mode** (`cargo xtask ci --full`): **95 PASS / 1 SKIP / 0 FAIL**. The SKIP is `audit-fault-injection-qemu-smoke` (`RUN_AUDIT_FAULT not set`); every other step, including the required manifest-boundary, OVMF-isolation, BPF-concurrency-boundary, process-lifecycle, Loom, and Miri checks, ran and passed.
  - **`RUN_AUDIT_FAULT=1` mode** (`RUN_AUDIT_FAULT=1 cargo xtask ci --full`): **96 PASS / 0 SKIP / 0 FAIL**. The fault-injection smoke and every required static, state-machine, model, Miri, build, and QEMU step run and pass; the full fault-injection coverage of physical allocation, mapper rollback, and exec/spawn failure paths is exercised.
...
- **Required local Miri evidence:** `miri-bpf-cloud` runs by default in the normal/full and extended local gates (`f5338dc`). At `c24f213`, `cargo miri test --locked -p kernel_bpf --no-default-features --features cloud-profile` passes **427 tests** (43 ignored) with zero UB; the default gate-recorded duration is **1055 seconds**. `--quick` is the documented iteration mode that omits Miri.
...
| H-06 | **Pending hosted evidence** | Workflow/toolchain/target/QEMU-gate defects are remediated. At `c24f213`, the default local gate passes 95 required steps with one conditional skip and fault mode passes 96/96, including required state-machine, Loom, and Miri evidence. Hosted jobs cannot start until GitHub billing/quota is restored. |
...
- [x] Pass the complete local required audit gate at `c24f213`: default mode is 95 PASS / 1 conditional SKIP / 0 FAIL; `RUN_AUDIT_FAULT=1` is 96 PASS / 0 SKIP / 0 FAIL. Both run through `cargo xtask ci --full`, including the required manifest-boundary, OVMF-VARS-isolation, BPF-concurrency-boundary, process-lifecycle, Loom, and Miri checks.
- [x] Require the Miri-clean cloud-profile BPF interpreter suite in the normal/full and extended local gates (`f5338dc`). At `c24f213`, all 427 selected tests pass with zero UB in a default-gate-recorded 1055 seconds; `--quick` remains the explicit iteration-only omission.
...
1. **Hosted H-06 evidence** — Contingent on GitHub Actions billing /
   ...
   “Hosted H-06 evidence” line in this document. At `c24f213`, the local
   default gate (95 PASS / 0 FAIL / 1 SKIP) and fault-mode gate (96 PASS /
...
| Done | Default-gate BPF Miri | `kernel/crates/kernel_bpf`, audit gate | Test/CI | Landed at `f5338dc` and verified at `c24f213`: the required `miri-setup` and `miri-bpf-cloud` steps run the existing cloud-profile suite and fail the normal/full gate on UB; 427 tests pass (43 ignored) | Default gate-recorded warm-host runtime was 1055 seconds. `--quick` deliberately omits Miri for iteration; it is not release evidence. |
| Done | BPF concurrency model boundary | `kernel/src/bpf`, `kernel_bpf` | Test/model | Four required Loom tests exercise the genuine `EpochSnapshot` reader/publisher/reclamation boundary; `bpf-concurrency-boundary-static` enforces mutex-serialized, exclusive-borrow handle generations (`558ee19`, reverified at `c24f213`) | No additional handle Loom wrapper is valid while handle state is private `BpfManager` state behind one production mutex. Reopen only if production synchronization changes. |
...
| 6 | Process-manager split | `kernel/src/mcore/mtask/process` | Kernel refactor | Process tree, file-descriptor state, construction/fork, and remaining exec orchestration invariants live in separately reviewed modules with unchanged ABI and passing gate | Executable/image, exit/wait lifecycle, and interruptible-sleep state are now extracted and gated (`c24f213`); the construction/fork/exec shell in `mod.rs` remains oversized and may enable the child-exit fixture seam when further split. |
| 7 | Uniform error policy | boot, syscall, VM, VFS, BPF, platform owners | Architecture + kernel refactor | Every supported boundary conforms to ADR-0002 with typed errors and explicit panic/halt policy; conformance checks cover remaining boot and driver edges | Existing typed VM/VFS/exec errors are partial evidence, not uniform enforcement. |
| 8 | Supported target and entrypoint cleanup | root manifests, `ci/targets.toml`, platform owners | Build/architecture | One generated supported matrix names every shipped entrypoint; parallel RISC-V experiments are removed or relocated outside shipped targets | Preserve ADR-0003 and current generated inventory. Do not delete an entrypoint until artifact ownership is established by the canonical manifest. |
| 9 | Artifact pinning and shipped-image provenance | build scripts, Limine/OVMF/Pi image owners | Build/release | Generated shipped-image and provenance tables contain source tag, hash, toolchain, and selection rule for every image; Pi selection no longer depends on mtime | Existing pinned primary OVMF and Limine inputs are partial. A second OVMF is not added solely to recreate the historical ring-3 symptom. |
| 10 | Broader deterministic fault injection | physical allocator, mapping, exec/spawn, remaining allocation owners | Test/kernel | Fail-after-N sweeps cover the remaining allocation/mapping transactions and prove rollback/no-leak invariants at each supported failure boundary | Existing physical allocation, mapper, and exec/spawn coverage remains complete for its stated scope. Extend by owned transaction, not a global failure switch. |
| 11 | Coverage and mutation budgets | all first-party crates, parser/verifier owners | Test/quality | Per-crate line/branch reports are published from the canonical manifest; parser/verifier mutation thresholds are versioned and enforced | Depends on the canonical boundary and stable test commands. Coverage numbers without an enumerated crate denominator do not close this row. |
...
## Remediation checklist (current branch, assessed through `c24f213`)
...
- [~] Split oversized process, BPF manager, verifier, signing, and actuation
  ...
  extracted and gated. Process executable/image and exit/wait lifecycle are
  now separate modules with a directly tested interruptible-sleep state, but
  the remaining construction/fork/exec shell is still oversized.
...
- [~] Pin mutable OVMF/Limine inputs and remove mtime-based artifact
  selection. OVMF and Limine are pinned and verified, and QEMU now treats the
  pinned OVMF VARS file as an immutable template via `snapshot=on`; the Pi
  artifact selection cleanup remains open.
- [~] Enforce unsafe-ledger, documentation-link, command-smoke, and provenance
  ...
  OVMF-VARS isolation check, and broad local audit gate are enforced;
  documentation-link and complete provenance enforcement remain incomplete.
