...
| 13 | Naming, benchmark, and documentation-link cleanup | product/docs owners, benchmark owners, release gate | Documentation + release | Remaining Axiom/AxiomOS/axiom-ebpf drift is resolved; benchmark tables include commit, toolchain, raw-log location, and artifact hashes; required links and commands are checked by the gate | **Partial.** Required `documentation-links`, `product-naming-static`, and `benchmark-provenance-static` steps validate local links, active naming, and attributable benchmark campaigns. The current host-verifier campaign records commit/toolchain/host, tracked raw output, Cargo-reported executable hash, and immutable input hashes; unsupported hardware/QEMU/Linux claims are archived. A bounded command-smoke contract remains open. |
...
- [~] Add deterministic fail-after-N allocation and mapping fault injection.
  ...
  append reservations, authorization-grant reservation, and pinned-map path
  reservation are covered; the broader allocation/mapping surface is not yet
  exhausted.
...
- [~] Enforce unsafe-ledger, documentation-link, command-smoke, and provenance
  ...
  audit gate are enforced. Repository-local Markdown targets are now enforced
  by the required `documentation-links` step; bounded command-smoke coverage
  remains incomplete.
