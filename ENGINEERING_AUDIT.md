...
- **Post-audit update (2026-07-13) — current `audit/dev-remediation` branch state:** the historical finding text below remains the evidence for audited commit `661d5ede6331c5ee62d6642451ce63ce1e0d5adf`; this amendment records subsequent remediation without silently rewriting that snapshot.
  ...
  - **Material partial remediation:** C-07 adds checked map allocation, hard global and per-owner object/byte/dimension quotas charged before allocation, PID ownership checks across the userspace map/program lifecycle, owner-filtered verifier map access, pin/unpin, detach/unload/destroy, synchronous busy checks, and scheduler-owned process-exit reclamation that waits for in-flight program snapshots. Raw IDs remain finite tombstoned vector indexes without generation/refcount handles. H-02 fixes clock units, overflow-safe conversion, clock IDs, and timespec validation but still polls for sleep; H-03 removes per-fire heap allocation and caps fanout but still takes a global runtime/manager lock; H-04 now isolates BPF objects by owning PID but still lacks credentials/capabilities and signed-only key provisioning.
  ...
  - **Still open:** C-07 generation-safe reusable handles, H-02 timer wait queues, H-03 lock-free/RCU dispatch, and H-04 credentials/capabilities plus signed-only key provisioning remain release work. The engineering score remains **3/10** and production readiness remains **NO-GO**.
