...
- **Implementation re-audited at:** `a409522` (completed process-invariant split) on top of the previously recorded exec/spawn rollback, mapping transaction, QEMU, concurrency-model, and required-Miri work
...
- **Local required gate evidence at `a409522`:**
  - **Default mode** (`cargo xtask ci --full`): **95 PASS / 1 SKIP / 0 FAIL**. The SKIP is `audit-fault-injection-qemu-smoke` (`RUN_AUDIT_FAULT not set`); every other step, including the required manifest-boundary, OVMF-isolation, BPF-concurrency-boundary, process-module-boundary, Loom, and Miri checks, ran and passed.
...
- **Required local Miri evidence:** `miri-bpf-cloud` runs by default in the normal/full and extended local gates (`f5338dc`). At `a409522`, `cargo miri test --locked -p kernel_bpf --no-default-features --features cloud-profile` passes **427 tests** (43 ignored) with zero UB; the default gate-recorded duration is **1236 seconds**. `--quick` is the documented iteration mode that omits Miri.
