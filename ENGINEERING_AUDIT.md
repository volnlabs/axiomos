...
- **Implementation re-audited at:** `1fcf1a9` (canonical shipped-image provenance and exact artifact selection) on top of the completed supported-target, typed-error, process-invariant, exec/spawn rollback, mapping transaction, QEMU, concurrency-model, and required-Miri work
...
- **Local required gate evidence at `1fcf1a9`:**
  - **Default mode** (`cargo xtask ci --full`): **98 PASS / 1 SKIP / 0 FAIL**. The SKIP is `audit-fault-injection-qemu-smoke` (`RUN_AUDIT_FAULT not set`); every other step, including the required artifact-provenance, target-boundary, error-policy, manifest-boundary, OVMF-isolation, BPF-concurrency-boundary, process-module-boundary, Loom, Miri, and QEMU checks, ran and passed.
  - **`RUN_AUDIT_FAULT=1` mode** (`RUN_AUDIT_FAULT=1 cargo xtask ci --full`): **99 PASS / 0 SKIP / 0 FAIL**. The fault-injection smoke and every required static, state-machine, model, Miri, build, and QEMU step run and pass; the full fault-injection coverage of physical allocation, mapper rollback, and exec/spawn failure paths is exercised.
...
- **Required local Miri evidence:** `miri-bpf-cloud` runs by default in the normal/full and extended local gates (`f5338dc`). At `1fcf1a9`, `cargo miri test --locked -p kernel_bpf --no-default-features --features cloud-profile` passes **427 tests** (43 ignored) with zero UB in both full modes. `--quick` is the documented iteration mode that omits Miri.
...
| H-06 | **Pending hosted evidence** | Workflow/toolchain/target/QEMU-gate defects are remediated. At `1fcf1a9`, the default local gate passes 98 required steps with one conditional skip and fault mode passes 99/99, including required state-machine, Loom, Miri, and artifact-provenance evidence. Hosted jobs cannot start until GitHub billing/quota is restored. |
...
- [x] Pass the complete local required audit gate at `1fcf1a9`: default mode is 98 PASS / 1 conditional SKIP / 0 FAIL; `RUN_AUDIT_FAULT=1` is 99 PASS / 0 SKIP / 0 FAIL. Both run through `cargo xtask ci --full`, including the required artifact-provenance, target-boundary, manifest-boundary, OVMF-VARS-isolation, BPF-concurrency-boundary, process-module-boundary, Loom, and Miri checks.
...
1. **Hosted H-06 evidence** — Contingent on GitHub Actions billing /
   ...
   “Hosted H-06 evidence” line in this document. At `1fcf1a9`, the local
   default gate (98 PASS / 0 FAIL / 1 SKIP) and fault-mode gate (99 PASS /
...
## Remediation checklist (current branch, assessed through `1fcf1a9`)
