...
- **Post-audit update (2026-07-13) — current `audit/dev-remediation` branch state:** the historical finding text below remains the evidence for audited commit `661d5ede6331c5ee62d6642451ce63ce1e0d5adf`; this amendment records subsequent remediation without silently rewriting that snapshot.
  ...
  - **Material partial remediation:** C-07 adds checked map allocation, hard global object/byte/dimension quotas, pin/unpin, detach/unload/destroy, and synchronous busy checks, but not per-process charging or generation handles; H-01 has an architecture-neutral user-exception result, scheduler-owned teardown, private x86 writable fork frames, and two-vCPU fault/exit/wait probes, but `execve` still holds a scheduler borrow across image replacement and lacks an end-to-end exec probe; H-02 fixes clock units, overflow-safe conversion, clock IDs, and timespec validation but still polls for sleep; H-03 removes per-fire heap allocation and caps fanout but still takes a global runtime/manager lock.
  - **CI and governance:** H-06 workflow fixes cover disk/OVMF discovery, explicit embedded targets, Lean PATH setup, the expanded host crate/profile matrix, ELF fuzzing, and a QEMU gate that requires boot, `EFAULT`, `ENOSYS`, and two-CPU TLB-shootdown markers; Q-04 now has a generated, exact-fingerprint unsafe ledger with every first-party Rust `unsafe` site assigned to an owner/invariant/test obligation. Local release build, both kernel targets, both complete BPF profiles, strict BPF Clippy, the focused syscall/user-memory/time/ELF suites, and the release QEMU smoke are green. Hosted GitHub Actions have not been rerun.
  - **Still open:** H-01 lifecycle integration, H-02 timer wait queues, H-03 lock-free/RCU dispatch, and H-04 credentials/capabilities plus signed-only key provisioning remain release work. The engineering score remains **3/10** and production readiness remains **NO-GO**.
