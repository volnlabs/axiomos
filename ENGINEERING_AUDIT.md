...
| 11 | Coverage and mutation budgets | all first-party crates, parser/verifier owners | Test/quality | Per-crate line/branch reports are published from the canonical manifest; parser/verifier mutation thresholds are versioned and enforced | **Partial.** `ci/quality.toml` enumerates all 49 canonical components and the required `quality-boundary-static` check rejects denominator or budget drift. Actual baselines are enforced for `kernel_bpf` (77.71% lines / 63.43% branches), `kernel_elfloader` (83.29% / 66.67%), `kernel_map_transaction` (94.35% / 100.00%), `kernel_physical_memory` with `fault-injection` enabled (94.71% / 77.08%), `kernel_time` (96.76% / 88.24%), and `kernel_usermem` (98.40% / 86.67%); mutation floors are enforced for the BPF verifier (77.42%), ELF parser/loader (81.71%), full `shrike_link` protocol/state-machine crate (92.26%), and bounded physical-memory `region.rs` scope (93.55%), with exact per-mutant evidence for both newer campaigns. The remaining `deferred-host` components still need measured coverage baselines before this row closes. |
...
- [~] Publish per-crate line/branch coverage and add parser/verifier mutation
  ...
  required line/branch baselines cover `kernel_bpf`, `kernel_elfloader`,
  `kernel_map_transaction`, `kernel_physical_memory` under `fault-injection`,
  `kernel_time`, and `kernel_usermem`; versioned mutation floors cover the BPF
  verifier, ELF parser/loader, full `shrike_link` protocol/state-machine crate,
  and bounded physical-memory `region.rs` scope. The remaining `deferred-host`
