...
- [~] Publish per-crate line/branch coverage and add parser/verifier mutation
  ...
  required line/branch baselines cover `kernel_bpf`, `kernel_elfloader`, and
  `kernel_map_transaction`, and versioned mutation floors cover the BPF
  verifier and ELF parser/loader. The remaining `deferred-host` components
  still need measured baselines.
