...
- **Local required gate evidence at `558ee19`:**
  - **Default mode** (`cargo xtask ci --full`): **93 PASS / 1 SKIP / 0 FAIL**. The SKIP is `audit-fault-injection-qemu-smoke` (`RUN_AUDIT_FAULT not set`); every other step, including the required manifest-boundary, OVMF-isolation, BPF-concurrency-boundary, Loom, and Miri checks, ran and passed.
  - **`RUN_AUDIT_FAULT=1` mode** (`RUN_AUDIT_FAULT=1 cargo xtask ci --full`): **94 PASS / 0 SKIP / 0 FAIL**. The fault-injection smoke and every required static, model, Miri, build, and QEMU step run and pass; the full fault-injection coverage of physical allocation, mapper rollback, and exec/spawn failure paths is exercised.
...
- **Required local Miri evidence:** `miri-bpf-cloud` runs by default in the normal/full and extended local gates (`f5338dc`). At `558ee19`, `cargo miri test --locked -p kernel_bpf --no-default-features --features cloud-profile` passes **427 tests** (43 ignored) with zero UB; the default gate-recorded duration is **1052 seconds**. `--quick` is the documented iteration mode that omits Miri.
...
| H-06 | **Pending hosted evidence** | Workflow/toolchain/target/QEMU-gate defects are remediated. At `558ee19`, the default local gate passes 93 required steps with one conditional skip and fault mode passes 94/94, including required Loom and Miri evidence. Hosted jobs cannot start until GitHub billing/quota is restored. |
...
- [x] Pass the complete local required audit gate at `558ee19`: default mode is 93 PASS / 1 conditional SKIP / 0 FAIL; `RUN_AUDIT_FAULT=1` is 94 PASS / 0 SKIP / 0 FAIL. Both run through `cargo xtask ci --full`, including the required manifest-boundary, OVMF-VARS-isolation, BPF-concurrency-boundary, Loom, and Miri checks.
- [x] Require the Miri-clean cloud-profile BPF interpreter suite in the normal/full and extended local gates (`f5338dc`). At `558ee19`, all 427 selected tests pass with zero UB in a default-gate-recorded 1052 seconds; `--quick` remains the explicit iteration-only omission.
...
1. **Hosted H-06 evidence** — Contingent on GitHub Actions billing /
   ...
   “Hosted H-06 evidence” line in this document. At `558ee19`, the local
   default gate (93 PASS / 0 FAIL / 1 SKIP) and fault-mode gate (94 PASS /
...
| Done | Default-gate BPF Miri | `kernel/crates/kernel_bpf`, audit gate | Test/CI | Landed at `f5338dc` and verified at `558ee19`: the required `miri-setup` and `miri-bpf-cloud` steps run the existing cloud-profile suite and fail the normal/full gate on UB; 427 tests pass (43 ignored) | Default gate-recorded warm-host runtime was 1052 seconds. `--quick` deliberately omits Miri for iteration; it is not release evidence. |
| Done | BPF concurrency model boundary | `kernel/src/bpf`, `kernel_bpf` | Test/model | Four required Loom tests exercise the genuine `EpochSnapshot` reader/publisher/reclamation boundary; `bpf-concurrency-boundary-static` enforces mutex-serialized, exclusive-borrow handle generations (`558ee19`) | No additional handle Loom wrapper is valid while handle state is private `BpfManager` state behind one production mutex. Reopen only if production synchronization changes. |
...
| 6 | Process-manager split | `kernel/src/mcore/mtask/process` | Kernel refactor | Process tree, file-descriptor state, exit/wait lifecycle, and exec/image invariants live in separately reviewed modules with unchanged ABI and passing gate | BPF, verifier, signing, and actuation splits already landed. The process manager is the remaining oversized production module and may enable the child-exit fixture seam. |
...
## Remediation checklist (current branch, assessed through `558ee19`)
