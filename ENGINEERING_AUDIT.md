...
- [~] Publish per-crate line/branch coverage and add parser/verifier mutation
  ...
  required line/branch baselines cover 14 host-testable components, including
  `xtask`, `kernel_abi`, strengthened devfs/syscall/virtual-memory suites,
  `kernel_vfs`, `shrike_link`, and standalone `rk_bridge`; versioned mutation
  floors cover the BPF verifier, ELF parser/loader, full `shrike_link`
  protocol/state-machine crate, and bounded physical-memory `region.rs` scope.
  Eight `deferred-host` components still need measured coverage baselines.
