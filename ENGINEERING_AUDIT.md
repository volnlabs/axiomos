...
- **Implementation re-audited at:** `3da8d8e` (deterministic BPF-handle reservation-failure coverage) on top of the completed shipped-image provenance, supported-target, typed-error, process-invariant, exec/spawn rollback, mapping transaction, QEMU, concurrency-model, and required-Miri work
...
- **Local required gate evidence at `3da8d8e`:**
  - **Default mode** (`scripts/verify-engineering-audit.sh`): **101 PASS / 1 SKIP / 0 FAIL**. The SKIP is `audit-fault-injection-qemu-smoke` (`RUN_AUDIT_FAULT not set`); every other step, including the required BPF-handle reservation-failure, artifact-provenance, target-boundary, error-policy, manifest-boundary, OVMF-isolation, BPF-concurrency-boundary, process-module-boundary, Loom, Miri, and QEMU checks, ran and passed.
  - **`RUN_AUDIT_FAULT=1` mode** (`RUN_AUDIT_FAULT=1 scripts/verify-engineering-audit.sh`): **102 PASS / 0 SKIP / 0 FAIL**. The fault-injection smoke and every required static, state-machine, model, Miri, build, and QEMU step run and pass; deterministic coverage includes physical allocation, mapper rollback, exec/spawn failure, and both BPF handle-table append reservation points.
...
- **Required local Miri evidence:** `miri-bpf-cloud` runs by default in the normal/full and extended local gates (`f5338dc`). At `3da8d8e`, `cargo miri test --locked -p kernel_bpf --no-default-features --features cloud-profile` passes **427 tests** (43 ignored) with zero UB in both full modes. `--quick` is the documented iteration mode that omits Miri.
...
| H-06 | **Pending hosted evidence** | Workflow/toolchain/target/QEMU-gate defects are remediated. At `3da8d8e`, the default local gate passes 101 required steps with one conditional skip and fault mode passes 102/102, including required state-machine, fault-injection, Loom, Miri, and artifact-provenance evidence. Hosted jobs cannot start until GitHub billing/quota is restored. |
...
| T-01 / T-02 / T-03 / T-04 | **Partial** | At `3da8d8e`, the default local gate passes 101 required steps with one conditional skip and fault mode passes 102/102; these close the original highest-risk gaps and add fault-injection and artifact-provenance coverage. Deterministic fault boundaries now cover: (1) physical allocation through the typed `armed(budget, f)` controller and QEMU probe; (2) mapper rollback through fixed-capacity `MapRangeTransaction`; (3) exec/spawn rollback through typed `ExecveError` and `CountingMemoryApi`; and (4) BPF handle-table append at both `Vec::try_reserve` points through the production `handles.rs` algorithm, proving no slot/generation publication and exactly-once value drop before recovery. **What these prove**: rollback and ownership correctness at those named boundaries. **What they do NOT prove**: every remaining allocation transaction, sustained userspace stability past `QEMU_BOOT_OK`, physical firmware behavior, or independent unsafe/concurrency review. Open: additional owned allocation transactions, physical HIL, wait-channel I/O fault-injection, coverage/mutation budgets, and independent review. |
...
- [x] Pass the complete local required audit gate at `3da8d8e`: default mode is 101 PASS / 1 conditional SKIP / 0 FAIL; `RUN_AUDIT_FAULT=1` is 102 PASS / 0 SKIP / 0 FAIL. Both run through `scripts/verify-engineering-audit.sh`, including the required BPF-handle fault-injection, artifact-provenance, target-boundary, manifest-boundary, OVMF-VARS-isolation, BPF-concurrency-boundary, process-module-boundary, Loom, and Miri checks.
...
1. **Hosted H-06 evidence** — Contingent on GitHub Actions billing /
   ...
   “Hosted H-06 evidence” line in this document. At `3da8d8e`, the local
   default gate (101 PASS / 0 FAIL / 1 SKIP) and fault-mode gate (102 PASS /
...
## Remediation checklist (current branch, assessed through `3da8d8e`)
...
- [~] Add deterministic fail-after-N allocation and mapping fault injection.
  Physical allocation, mapping rollback, exec/spawn, and both BPF handle-table
  append reservations are covered; the broader allocation/mapping surface is
  not yet exhausted.
...
- [~] Enforce unsafe-ledger, documentation-link, command-smoke, and provenance
  ...
  audit gate are enforced; documentation-link enforcement remains incomplete.
