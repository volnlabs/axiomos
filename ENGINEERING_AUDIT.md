...
- **Implementation re-audited at:** `1c50faa` (typed boot/driver error-policy enforcement) on top of the completed process-invariant split and the previously recorded exec/spawn rollback, mapping transaction, QEMU, concurrency-model, and required-Miri work
...
- **Local required gate evidence at `1c50faa`:**
  - **Default mode** (`cargo xtask ci --full`): **96 PASS / 1 SKIP / 0 FAIL**. The SKIP is `audit-fault-injection-qemu-smoke` (`RUN_AUDIT_FAULT not set`); every other step, including the required error-policy, manifest-boundary, OVMF-isolation, BPF-concurrency-boundary, process-module-boundary, Loom, Miri, and QEMU checks, ran and passed.
  - **`RUN_AUDIT_FAULT=1` mode** (`RUN_AUDIT_FAULT=1 cargo xtask ci --full`): **97 PASS / 0 SKIP / 0 FAIL**. The fault-injection smoke and every required static, state-machine, model, Miri, build, and QEMU step run and pass; the full fault-injection coverage of physical allocation, mapper rollback, and exec/spawn failure paths is exercised.
...
- **Required local Miri evidence:** `miri-bpf-cloud` runs by default in the normal/full and extended local gates (`f5338dc`). At `1c50faa`, `cargo miri test --locked -p kernel_bpf --no-default-features --features cloud-profile` passes **427 tests** (43 ignored) with zero UB in both full modes. `--quick` is the documented iteration mode that omits Miri.
...
| H-06 | **Pending hosted evidence** | Workflow/toolchain/target/QEMU-gate defects are remediated. At `a409522`, the default local gate passes 95 required steps with one conditional skip and fault mode passes 96/96, including required state-machine, Loom, and Miri evidence. Hosted jobs cannot start until GitHub billing/quota is restored. |
...
- [x] Pass the complete local required audit gate at `a409522`: default mode is 95 PASS / 1 conditional SKIP / 0 FAIL; `RUN_AUDIT_FAULT=1` is 96 PASS / 0 SKIP / 0 FAIL. Both run through `cargo xtask ci --full`, including the required manifest-boundary, OVMF-VARS-isolation, BPF-concurrency-boundary, process-module-boundary, Loom, and Miri checks.
...
1. **Hosted H-06 evidence** — Contingent on GitHub Actions billing /
   ...
   “Hosted H-06 evidence” line in this document. At `a409522`, the local
...
| 7 | Uniform error policy | boot, syscall, VM, VFS, BPF, platform owners | Architecture + kernel refactor | Every supported boundary conforms to ADR-0002 with typed errors and explicit panic/halt policy | All designated boot, filesystem, driver, address, process fork/exec, and AArch64 paging/DTB boundaries now expose typed errors under the required `error-policy-static` check. AArch64 copy-on-write replacement also restores the old mapping if private-page publication fails. Full default/fault gate evidence is still required before closure. |
...
## Remediation checklist (current branch, assessed through `a409522`)
...
- [~] Define consistent kernel, syscall, VM, BPF, and boot error policies.
  ...
  publication fails. Full default/fault gate evidence is still pending before
  this item can close.
