...
The planning sources are in the sibling `axiom-lab` repository: [release roadmap][roadmap], [v1 vision][vision], [north star][northstar], [research program][research], [runtime brainstorm][brainstorm], and [superseded critical path][critical]. I apply the requested precedence. Existing edits to `docs/plans/README.md` and the untracked v0.5 engineering draft were left untouched and are not treated as PR #35 implementation.
