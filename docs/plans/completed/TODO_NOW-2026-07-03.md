...
> **Status amendment — 2026-07-18:** This completed audit plan remains a
> point-in-time record. Use the active [v0.5.0-alpha.1 release plan](../active/v0.5-alpha-release.md)
> for current release tracking. The GPIO dispatch implementation and portable
> FPGA final-PWM gate now exist in source (`fdd5bf0`), and the latter is
> simulated by the full local gate and a dedicated hosted job (`92e7bd3`).
> Physical RPi5/RP2040/FPGA HIL, a board project, constraints, and a bitstream
> are still open; source-level completion is not physical assurance.
...
### H3 — GPIO-edge → BPF dispatch is implemented in source, not proven on hardware
- **Status update (2026-07-18):** The production GPIO IRQ →
  `execute_program` route and immutable GPIO snapshots are present in source.
  IIO/kprobe attach shims remain separate limitations. The sense→decide→act
  loop has not run on physical hardware, so this item remains HIL-blocked.
...
- **Description:** `shrike_link` (the codec + fail-safe watchdog) is well tested
  on host (~52 tests). But `firmware/shrike/rp2040/` (motor.rs, control.rs,
...
- **Files:** `firmware/shrike/rp2040/src/{main,motor,control}.rs`.
...
- FPGA e-stop interlock: portable final-PWM gate RTL and a self-checking
  Icarus testbench landed in `fdd5bf0`; the full local gate and a dedicated
  hosted job run the simulation (`92e7bd3`). Board project, pin constraints,
  synthesized bitstream, timing closure, and physical HIL remain open. Do not
  claim a deployed hardware-non-bypassable layer until those artifacts exist.
...
Updated triage (2026-07-18, preserving the 2026-07-03 findings): **C2 is fixed** (#181 — the real IRQ-context
...
v0.4. v0.4 source work now also includes the portable final-PWM safety RTL;
the remaining v0.4 requirements are board integration and HIL evidence, which
cannot be closed at a desk. The codebase is architecturally strong (clean crate
DAG, sound verifier design, genuinely good `shrike_link` safety logic); the one
remaining correctness item (C3) and the claim/allocation mismatches (H1 static
memory) are the cleanup before v0.5.
