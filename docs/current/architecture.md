...
- **Scheduling:** one run queue exists per CPU, enqueue uses the task's recorded
  ...
  [ADR-0001](../adr/0001-runtime-scheduling-locking.md). A wakeup IPI remains an
...
- **BPF:** the kernel binary owns credentials, handles, quotas, attachment
  ...
  complete security boundary is normative in [BPF trust and lifecycle](bpf-trust.md).
