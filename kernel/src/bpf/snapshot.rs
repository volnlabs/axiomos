...
pub(crate) struct EpochSnapshot<T> {
    current: AtomicPtr<T>,
    epoch: AtomicUsize,
...
impl<T> EpochSnapshot<T> {
    /// Construct an unpublished snapshot suitable for static initialization.
    ...
    pub(crate) fn read(&self) -> Option<EpochReadGuard<'_, T>> {
        loop {
    ...
    pub(crate) fn publish(&self, next: Box<T>) {
...
impl<T> Drop for EpochSnapshot<T> {
    fn drop(&mut self) {
        ...
        if !current.is_null() {
            // SAFETY: exclusive access proves there are no safe outstanding
            // guards or publishers. `current` was created by Box::into_raw.
            unsafe {
...
pub(crate) struct EpochReadGuard<'a, T> {
