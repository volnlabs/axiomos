...
- Existing build, run, deploy, debug, and audit behavior is preserved during
  migration; temporary compatibility wrappers are removed after caller migration.
...
- [x] Run the full supported audit profile after all moves, then remove only the
      compatibility paths proven to have no remaining callers. On `a327533`,
      `cargo xtask check all --profile full --format human` passed 112 checks
      with 0 failures and 2 intentional skips; all zero-caller flat wrappers
      were removed before the final run. The final fault-injection quick profile
      passed 76 checks with 0 failures and 0 skips.
